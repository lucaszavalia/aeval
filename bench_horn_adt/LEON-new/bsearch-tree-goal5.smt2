...
(declare-fun tremove-all (Tree Lst Tree) Bool)
(assert (forall ((x Tree)) (tremove-all x nil x)))
(assert (forall ((x Tree) (n Int) (l Lst) (y Tree) (z Tree)) 
	(=> (and (tremove x n y) (tremove-all y l z)) (tremove-all x (cons n l) z))))

; conjecture
(assert (forall ((l Lst) (t Tree) (r Tree) (ts Int) (rs Int))
	(=> (and (tremove-all t l r) (tsize t ts) (tsize r rs) (not (<= rs ts))) false))); G-bsearch-tree-5 
